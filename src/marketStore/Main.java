...
//        testGold700();
//        testBronzeNegative();
//        testBronzeNegativePurchaseValue();
    }

    private static void testBronzeNegativePurchaseValue() {
        double turnover = 1;
    ...
    private static void testBronzeNegative() {
        double turnover = -1;
        Card bronzeCard = new Bronze(turnover);
        double valueOfPurchase = 150;
        Purchase purchase = new Purchase(valueOfPurchase,bronzeCard);
    ...
    private static void testGold700() {
        double turnover = 700;
        Card goldCard = new Gold(turnover);
    ...
    private static void testGold1500() {
    ...
    private static void testSilver() {
        double turnover = 600;
        Card silverCard = new Silver(turnover);
...
Each card stores information about its owner, the turnover for the previous month and the initial discount rate.
 With each card you can calculate the discount of the current purchase.
  In general this is done using the formula: valueOfPurchase * discountRate.

The bronze card comes with no discount rate if the turnover for the previous month
is below $100. If it is between $100 and $300, the discount rate is 1%. And if it is above $300, the rate is 2.5%.

The silver card comes with an initial discount rate of 2%. In case, the turnover is above $300, the rate is 3.5%.

The gold card comes with an initial discount rate of 2%. The discount rate grows 1% for each $100 from
the turnover, capping at 10%.
...
in a preferred object-oriented programming language
for a preferred platform - mobile, console, web, desktop
Deliverables:
a link to the source code in version control system (GitHub, Bitbucket, GitLab, etc.)
or a cloud storage (Google Drive, OneDrive, iCloud, etc.).
 */